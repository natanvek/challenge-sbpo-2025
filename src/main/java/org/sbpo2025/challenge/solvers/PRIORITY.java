...
public class PRIORITY extends Heuristica {
    public PRIORITY(List<Map<Integer, Integer>> _orders, List<Map<Integer, Integer>> _aisles, int _nItems,
            int _waveSizeLB,
            int _waveSizeUB) {
        super(_orders, _aisles, _nItems, _waveSizeLB, _waveSizeUB);
    }

    PriorityQueue<Heuristica.EfficientCart> priorityRanking;
    ...
    public void runRanking() {
        ...
        priorityRanking = new PriorityQueue<>(Comparator.comparingDouble(EfficientCart::getValue));
        ...
        for (Aisle p : aisles)
            for (int r = tope - 1; r >= 0; --r)
                for (EfficientCart m : rankings.get(r)) {
                    ...
                    insertCart(rankings2.get(r + 1), copia, 5);
                    if (copia.getCantItems() >= waveSizeLB)
                        insertCart(priorityRanking, copia, 10);
                }
    ...
    public void calcPriority() throws IloException {

        int[] priorityArr = new int[nAisles];
        IloCplex.BranchDirection[] directionsArr = new IloCplex.BranchDirection[nAisles];
        for (int i = 0; i < nAisles; i++) {
            directionsArr[i] = IloCplex.BranchDirection.Global;
            priorityArr[i] = 0;
        }

        int van = 0;
        for (EfficientCart c : priorityRanking) {
            for (Aisle a : c.getAisles()) {
                if (priorityArr[a.pos] == 0)
                ++van;
                priorityArr[a.pos]++;
                // if(priorityArr[a.pos] > priorityRanking.size() / 4)
                directionsArr[a.pos] = IloCplex.BranchDirection.Up;
            }
        }

        // for (int i = 0; i < nAisles; i++) {
        //     if (priorityArr[i] != priorityRanking.size()) {
        //         directionsArr[i] = IloCplex.BranchDirection.Global;
        //         priorityArr[i] = 0;
        //     } else {
        //         ++van;
        //         directionsArr[i] = IloCplex.BranchDirection.Up;
        //     }
        // }

        System.out.println("Van: " + van);

        cplex.setPriorities(aCP, priorityArr);
        cplex.setDirections(aCP, directionsArr);
    }
    ...
    public void setUpHCplex() throws IloException {
        ...
        chooseRange = cplex.addRange(0, nAislesChoose, 2);
