...
public class SuperRanking extends Heuristica {
    public SuperRanking(List<Map<Integer, Integer>> _orders, List<Map<Integer, Integer>> _aisles, int _nItems,
            int _waveSizeLB,
            int _waveSizeUB) {
    ...
    public void setUpCplex(StopWatch stopWatch) throws IloException {
        cplex.setOut(null);
        ...
        waveRange = cplex.addRange(0, waveSize, waveSizeUB);
        ...
        IloRange[] cons = new IloRange[nAisles];
        for (int i = 0; i < nAisles; i++)
            cons[i] = cplex.addEq(aCP[i], 0); // inicialmente todas = 0
        ...
        int contarPasillos = 0, va = 10;
        for (Aisle p : aisles) {
            if (++contarPasillos == va) {
                System.out.println("Pasillos: " + va);
                printElapsedTime(stopWatch);
                va += 10;
            }
            cons[p.pos].setBounds(1, 1);
            cplex.setParam(IloCplex.Param.TimeLimit, 0.001);

            for (int r = tope - 1; r >= 0; --r)
                for (EfficientCart m : rankings.get(r)) {
                    EfficientCart copia = new EfficientCart(m);
                    for (Aisle a : copia.my_aisles)
                        cons[a.pos].setBounds(1, 1);

                    copia.cantItems = 0;
                    if (!rankings.get(r + 1).isEmpty()) {
                        waveRange.setLB(rankings.get(r + 1).peek().getCantItems() + 1);
                    } else {
                        waveRange.setLB(0);
                    }
                    if (cplex.solve()) {
                        ...
                        if (mnrta < posiblerta && cplex.getValue(waveSize) >= waveSizeLB) {
                            System.out.println(
                                    "encontre solucion nAislesCP: " + cplex.getValue(nAislesCP) + " rta: "
                                            + posiblerta);
                            mnrta = posiblerta;
                            rtaAisles.clear();
                            rtaOrders.clear();
                            ...
                            tope = (int) Math.floor(1e-4 + waveSizeUB / mnrta);
                            System.out.println("nuevo tope: " + tope);
                            copia.cantItems = (int) (cplex.getValue(waveSize) + 1e-4);
