...
import org.sbpo2025.challenge.AisleCollection;
...
public class RankingAS extends Heuristica {
    public RankingAS(List<Map<Integer, Integer>> orders, List<Map<Integer, Integer>> aislesh, int nItems,
            int waveSizeLB,
            int waveSizeUB) {
        super(orders, aislesh, nItems, waveSizeLB, waveSizeUB);
    }

    public <AC extends AisleCollection> boolean insertCart(PriorityQueue<AC> queue, AC newCart, Integer regSize) {
        if (queue.size() < regSize) {
            queue.offer(newCart);
            return true;
        }

        if (queue.peek().isWorseThan(newCart)) {
            queue.poll();
            queue.offer(newCart);
            return true;
        } else {
